...
class SalesManController(http.Controller):
    ...
    def get_company(self, **kwargs):
        data = json.loads(request.httprequest.data)
        keys = {
            'id': lambda r: r.id if r.id else "",
            'name': lambda r: r.name if r.name else "",
            'code': lambda r: r.code if r.code else "",
        }
        return self.get_data_artifacts(data, 'res.company', keys=keys)

    @validate_token
    @http.route('/api/v1/branch', type='http', auth='none', methods=['GET'], csrf=False)
    def get_branch(self, **kwargs):
        data = json.loads(request.httprequest.data)
        keys = {
            'id': lambda r: r.id if r.id else "",
        ...
        return self.get_data_artifacts(data, 'multi.branch', keys=keys)
    ...
    def _generate_account_entry(self, user, data):
        ...
        if not company.exists():
            return self._error("Invalid company id provided. Kindly Check the company Api documentation for company ids")
        ...
        if journal.company_id.id != company.id:
            return self._error(f"Journal with name {journal.name} belongs to {journal.company_id.name}. Kindly ensure you set journal that match the company iD({company.id}), you are passing")
        ...
        for line in data['transactions']:
            ...
            if not account:
                return self._error(f"""Account with code {line.get('account_code')} not found:
                                    Note: Ensure the account exists and also matches company provided""")
