...
class OfficeDashboard(http.Controller):
	...
	@http.route(["/office-dashboard", "/office-dashboard/<string:search_request>", "/office-dashboard/type/<string:memo_type_param>"], type='http', auth='user', website=True, website_published=True)
	def office_dashboard(self, memo_type_param='document_request', search_request=False):
		...
		vals = {
			...
			"y_data_chart": json.dumps({'data': self.get_xy_data(memo_type_param)[0] or '[]'}),
	...
	def get_xy_data(self, memo_type_param):
				...
				for doc in document_folders:
					...
					if docu_ids:
						...
						if submitted_documents_document:
							# raise ValidationError(submitted_documents_document)
							department_submission += 1
						# else:
						# 	raise ValidationError('xxxxs')
				dept_document_ratio = department_submission * document_ratio # total to display 4 * 20 == 80
